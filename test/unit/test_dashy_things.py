...
def test_valid_pacprotocol_address():
    from pacprotocollib import is_valid_pacprotocol_address

    main = valid_pacprotocol_address()
    test = valid_pacprotocol_address('testnet')

    assert is_valid_pacprotocol_address(main) is True
    assert is_valid_pacprotocol_address(main, 'mainnet') is True
    assert is_valid_pacprotocol_address(main, 'testnet') is False

    assert is_valid_pacprotocol_address(test) is False
    assert is_valid_pacprotocol_address(test, 'mainnet') is False
    assert is_valid_pacprotocol_address(test, 'testnet') is True


def test_invalid_pacprotocol_address():
    from pacprotocollib import is_valid_pacprotocol_address

    main = invalid_pacprotocol_address()
    test = invalid_pacprotocol_address('testnet')

    assert is_valid_pacprotocol_address(main) is False
    assert is_valid_pacprotocol_address(main, 'mainnet') is False
    assert is_valid_pacprotocol_address(main, 'testnet') is False

    assert is_valid_pacprotocol_address(test) is False
    assert is_valid_pacprotocol_address(test, 'mainnet') is False
    assert is_valid_pacprotocol_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from pacprotocollib import elect_mn
...
def test_parse_masternode_status_vin():
    from pacprotocollib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % pacprotocollib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(pacprotocollib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(pacprotocollib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(pacprotocollib.blocks_to_seconds(16616)) == 2612035
