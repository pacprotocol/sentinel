"""
pacprotocold JSONRPC interface
"""
import sys
import os
...
class pacprotocolDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some pacprotocold methods
    ...
    @classmethod
    def from_pacprotocol_conf(self, pacprotocol_dot_conf):
        from pacprotocol_config import pacprotocolConfig
        config_text = pacprotocolConfig.slurp_config_file(pacprotocol_dot_conf)
        creds = pacprotocolConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = pacprotocollib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = pacprotocollib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = pacprotocollib.blocks_to_seconds(diff)
