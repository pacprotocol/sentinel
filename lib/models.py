...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, pacprotocold):
        golist = pacprotocold.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_pacprotocold(pacprotocold, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_pacprotocold(self, pacprotocold, rec):
        import decimal
        ...
        # exclude any invalid model data from pacprotocold...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from pacprotocold, with every run
        ...
        # get/create, then sync payment amounts, etc. from pacprotocold - pacprotocold is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(pacprotocold)
                return (govobj, None)
    ...
    def vote_delete(self, pacprotocold):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(pacprotocold, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, pacprotocold, signal, outcome):
        import pacprotocollib
        ...
        output = pacprotocold.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = pacprotocollib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(pacprotocold, signal)

    def sync_network_vote(self, pacprotocold, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = pacprotocold.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 pacprotocol addr, non-multisig
            if not pacprotocollib.is_valid_pacprotocol_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid pacprotocol address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # pacprotocol Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (pacprotocollib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not pacprotocollib.is_valid_pacprotocol_address(addr, config.network):
    ...
    def hash(self):
        import pacprotocollib
        return pacprotocollib.hashit(self.serialise())
