...
def has_pacprotocol_conf():
    import config
    import io

    valid_pacprotocol_conf = False

    # ensure pacprotocol_conf exists & readable
    #
    # if not, print a message stating that pacprotocol Core must be installed and
    # configured, including JSONRPC access in pacprotocol.conf
    try:
        f = io.open(config.pacprotocol_conf)
        valid_pacprotocol_conf = True
    except IOError as e:
        print(e)

    return valid_pacprotocol_conf
...
def main():
    ...
    if not has_pacprotocol_conf():
        print("pacprotocolCore must be installed and configured, including JSONRPC access in pacprotocol.conf")
        sys.exit(1)
