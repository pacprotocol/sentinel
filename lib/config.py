...
sentinel_cfg = pacprotocolConfig.tokenize(sentinel_config_file)
sentinel_version = "1.6.0"


def get_pacprotocol_conf():
    if sys.platform == 'win32':
        pacprotocol_conf = os.path.join(os.getenv('APPDATA'), "pacprotocol/pacprotocol.conf")
    else:
        home = os.environ.get('HOME')

        pacprotocol_conf = os.path.join(home, ".pacprotocol/pacprotocol.conf")
        if sys.platform == 'darwin':
            pacprotocol_conf = os.path.join(home, "Library/Application Support/pacprotocol/pacprotocol.conf")

    pacprotocol_conf = sentinel_cfg.get('pacprotocol_conf', pacprotocol_conf)

    return pacprotocol_conf
...
pacprotocol_conf = get_pacprotocol_conf()
network = get_network()
