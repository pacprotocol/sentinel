...
def is_valid_pacprotocol_address(address, network='mainnet'):
    ...
    pacprotocol_version = 140 if network == 'testnet' else 55
    ...
    try:
        ...
        # rescue from exception, not a valid pacprotocol address
        return False

    if (address_version != pacprotocol_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('pacprotocol.conf')
